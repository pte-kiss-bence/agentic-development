...
- **`## Estimation`** — a **rollup** of the epic's child-story estimates: the Σ of their `Eβ` ideal engineer-hours and Σ Story Points, per the epic-rollup rule in [`../pte-openspec-shared/ESTIMATION.md`](../pte-openspec-shared/ESTIMATION.md). Because the estimate lives on the stories (authored by `pte-openspec-to-stories`, which runs *after* this skill), on first mint this section is a **placeholder** — `_(rollup: minden story esztimálása után)_` — that the **estimation-rollup pass** (below) fills once the child cards exist and are estimated. If any child story lacks an estimate, mark the rollup incomplete (`⚠ nem minden story esztimált`) rather than guessing a total.

...
- **`## Epic`** — narrative from the spec's purpose/overview: what the capability is and who it serves.
...
- **`## Problem / Solution`** — the problem the spec solves and the solution shape, as **two stacked paragraphs**: a `**Probléma:** …` paragraph and a **separate** `**Megoldás:** …` paragraph with a **blank line between them**, so they render one under the other and never merge into one paragraph. Use the spelled-out, **bold** labels `**Probléma:**` / `**Megoldás:**` — never the `P:` / `S:` shorthand and never unbolded.
...
- **`## MVP and Out of Scope`** — the in-scope MVP bullets and the explicit Out-of-Scope bullets (replaces the old *Hatókör és nem-célok*). Label the two lists with **bold** `**MVP:**` and `**Out of Scope:**` on their own lines, each followed by its bullet list.
...
7. **Verify exhaustively.** Every source Requirement and Scenario is accounted for (covered by a story or explicitly out of scope); no Epic exceeds ~10 stories (split if it does); every story has a unique `STORY-…` ID and acceptance criteria; every map-table title matches the spec verbatim; the *User Stories* list and *Source Spec Reference* map sit inside the `<!-- pipeline-only:start -->`/`<!-- pipeline-only:end -->` fence, and every non-pipeline section uses its English heading. Report any Requirement or Scenario you could not place cleanly rather than guessing.

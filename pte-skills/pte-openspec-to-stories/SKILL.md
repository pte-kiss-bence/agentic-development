...
6. **Verify exhaustively.** Every `STORY-…` in every source epic has exactly one card; every card has the four Jira-visible sections (User Story / Context / BDD Test placeholder / Risks and Dependencies) plus the pipeline-only fence (`Parent epic:` line, Estimation, Dependency Edges) and no dropped section; every `## Estimation` carries a PERT base + derived SP (and the `⚠` split-warning wherever the `ESTIMATION.md` threshold trips); **every card satisfies INVEST** (all six letters — flag any story that does not, and reshape/split rather than ship it); every reused ID matches the epic verbatim (Cím + parent line); epic-less cards carry the self-map inside the pipeline-only fence and epic-driven cards do not. Report any story or Scenario you could not place cleanly rather than guessing.

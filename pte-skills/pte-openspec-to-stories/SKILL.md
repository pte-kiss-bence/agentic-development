...
- **`## BDD Test`** — a placeholder for the declarative Gherkin that `pte-openspec-bdd-tests` fills in place from the story's mapped `#### Scenario`s. **This Gherkin is the story's acceptance criteria** — the new schema drops the separate *Elfogadási kritériumok* section; the `#### Scenario` `WHEN`/`THEN` become the `Adott/Amikor/Akkor` steps of the test, not a duplicated prose list. Author this section as a marker only — do **not** write Gherkin here:
  ...
  ```
- **`## Estimation`** — the story's **reference base** estimate: a PERT three-point (`O`/`M`/`P` ideal engineer-hours → `Eβ`, `σ`) plus the derived Story Points, with the `⚠` split-warning note when it trips. **Do not guess the hours** — derive `O`/`M`/`P` by scoring the **weighted complexity rubric** (M-drivers + modifiers → `M`; σ-drivers → the O/P spread) in [`../pte-openspec-shared/ESTIMATION.md`](../pte-openspec-shared/ESTIMATION.md), which is the single source of truth for the rubric, the PERT formula, the Eβ→SP table, the 6h-day anchor, and the split-warning threshold. This is a **visible** section (the SP feeds Jira's *Story points* field on sync); the multiplier layer stays downstream and out of scope.
...
**Dropped from the old schema** (do not emit them): the *Elfogadási kritériumok* section (now the BDD Gherkin), the *INVEST-ellenőrzés* section, *Készenléti feltétel (DoR)* / *Elkészültségi feltétel (DoD)*, *Prioritás*, and — for epic-driven stories — the *Forrás-hivatkozás* trace row. (The old *Becslés* section is **not** dropped — it is **reintroduced** as `## Estimation` above, reshaped into the PERT + Story-Points reference base.) **Note:** only the printed *INVEST-ellenőrzés* section is dropped — INVEST itself is **not** dropped; it is now a mandatory rule (below), enforced without a section.

...
From `openspec/backlog/epics/EPIC-belso-hozzaferes.md` (produced by `pte-openspec-to-epics`), the `## User Stories` entry for this story:
...
Its *Source Spec Reference* row — held in the **epic's** pipeline-only fence, not in the card:
...
- **Self-carried map inside a pipeline-only fence** — with no epic to hold the *Source Reference* row, the card carries its own row (hidden from Jira by the fence) so `pte-openspec-bdd-tests` knows which `#### Scenario`s to turn into the card's BDD test.

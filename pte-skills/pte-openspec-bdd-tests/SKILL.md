...
description: Generate declarative BDD test scenarios (Gherkin) and embed them into the story cards' BDD teszt section. Use when the user wants BDD, Gherkin, or Cucumber scenarios for story cards produced by pte-openspec-to-stories, mentions "BDD tests from specs/stories", or wants spec scenarios turned into executable living documentation inside the story cards.
---

An OpenSpec spec already carries behaviour: each `### Requirement` holds `#### Scenario` blocks of `WHEN`/`THEN` bullets. That is BDD in embryo. This skill turns it into **declarative** Gherkin and writes it **into the story cards** — each card owns its own BDD test(s), so the story is a self-contained, testable unit. The Gherkin reads as living documentation and stays stable when the implementation changes, never leaking UI or procedural detail.

This is the **last stage of the linear pipeline** (`epics` → `stories` → **bdd-tests**). It produces no separate `.feature` tree: it fills the `BDD teszt` section that `pte-openspec-to-stories` left as a placeholder in each card, and touches nothing else.
...
- [ ] 8. Every card has Gherkin; every mapped Scenario present; unmappable bullets reported
```

1. **Resolve the source story cards.** Locate the cards (default `stories/`, configurable). If several epics' cards exist and it is unclear which to cover, list them with **AskUserQuestion**. **Fallback only when no cards exist:** if the target has story cards missing, say so and offer to run `pte-openspec-to-stories` first (preferred). Only if the user insists on standalone `.feature` output with no cards, generate Gherkin from the spec directly and flag it as a degraded, non-embedded path. Completion: the exact card set (or the flagged fallback) is named.

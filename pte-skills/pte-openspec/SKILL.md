...
2. **`pte-openspec-to-stories`** → `openspec/backlog/stories/` — expands each epic story into a lean, refinement-ready Agile story card (`Description`/`Context`/`BDD Test`/`Estimation`/`Risks and Dependencies`), one per `STORY-…`, carrying no trace row of its own. The `Estimation` is a PERT + Story-Points **reference base** (see `CONVENTIONS.md`). **CONSUMES** the epic's IDs. In **epic-less** mode (small standalone change, no epic) it instead mints its own `STORY-<capability-slug>-…` straight from the delta and self-carries its map. Leaves a **`BDD Test`** section as a placeholder for step 3.

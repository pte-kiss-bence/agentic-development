...
- **Dependency Edges block** — the machine-readable `## Dependency Edges` section (pipeline-only) each epic and story card owns, declaring its outbound dependencies from the card's own perspective. Four ASCII tags — `depends-on` / `blocks` / `relates-to` (a backticked target trace ID) and `external` (free-text, graph-only) — so the edges parse without reading Hungarian prose. The **single source of truth for edges**: `pte-openspec-dependency-graph` renders it as a Mermaid graph and `pte-openspec-jira-sync` mirrors it as real Jira issue links, while the Jira-visible *Risks and Dependencies* prose stays human narrative and is never parsed. `depends-on`/`blocks` are inverses (deduped to one edge); a dangling target is warned-and-skipped. Contract: [`CONVENTIONS.md` → The *Dependency Edges* block](../pte-skills/pte-openspec-shared/CONVENTIONS.md#the-dependency-edges-block--the-edge-contract).

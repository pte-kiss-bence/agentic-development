...
- **Full feature (planning chain)** — the change introduces a real capability worth decomposing into a backlog. Start the `pte-openspec` pipeline by invoking `pte-openspec-to-epics` in **mint mode** (planning-chain entry point, steps 1–3: epics → story cards → BDD tests), then `pte-openspec-tdd-execute` builds it test-first once planning is ready.
- **Single story (no new epic)** — the change is bigger than a tweak but doesn't merit an epic: one (or a few) stories' worth of work. Two sub-cases, decided by whether the affected capability already has an epic:
  - *Affects an existing epic (default for a change request, since a change modifies existing behaviour):* run `pte-openspec-to-epics` in **reconcile mode** to fold the new story under the epic the delta touches (reuses the `EPIC-…`, mints the new `STORY-…`), then `pte-openspec-to-stories` → `pte-openspec-bdd-tests` → `pte-openspec-tdd-execute`.
  - *No epic covers the capability:* run `pte-openspec-to-stories` in **epic-less mode** to author the standalone card(s) straight from the delta, then `pte-openspec-bdd-tests` → `pte-openspec-tdd-execute`. Skips `pte-openspec-to-epics` entirely.
- **Small change / bug fix (skip the planning chain)** — a tweak, small refactor, or bug that touches ~one capability and carries no product decision. Epics/stories/BDD would be pure ceremony, so go straight to the build: `pte-openspec-tdd-execute` (test-first — for a bug, this means a failing test that reproduces it, then the fix), or plain `/opsx:apply` when a test adds no value. For a bug whose root cause isn't already obvious, run the `diagnosing-bugs` skill first to pin down the cause, then feed that into the test-first fix. For bugs this whole path is the default; only escalate upward if the fix turns out to be a feature in disguise.
...
**The build step is always started by the user, never by the agent.** `pte-openspec-tdd-execute` is user-invoked (`disable-model-invocation`, see ADR 0005): wherever a path above ends in it, that step means *instruct the user to type* `/pte-openspec-tdd-execute` — do not attempt to invoke it via the Skill tool (it will be refused, even right after the user picked the build option), and do not simulate it by hand-following its file; wait for the user to run it.
